...
class JobBigQueryRepository(
    ...
    override fun getLandingPageData(): LandingPageDto {
        ...
        return LandingPageDto(
                globalStats = GlobalStatsDto(totalVacancies, remotePct, hybridPct, topTechName),
                topTech = topTechList,
    ...
    override fun getCompanyProfile(companyId: String): CompanyProfilePageDto {
        ...
        val logo =
                if (detRow?.get("logoUrl")?.isNull == false) detRow.get("logoUrl").stringValue
                else ""
        val website =
                if (detRow?.get("website")?.isNull == false) detRow.get("website").stringValue
                else ""
        val emps =
        ...
        val ind =
                if (detRow?.get("industries")?.isNull == false) detRow.get("industries").stringValue
                else ""
        val desc =
        ...
        val topModel =
                if (aggRow?.get("topModel")?.isNull == false) aggRow.get("topModel").stringValue
