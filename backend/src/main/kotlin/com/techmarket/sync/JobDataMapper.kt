    ...
    fun mapSyncData(apifyJobs: List<ApifyJobDto>): MappedSyncData {
        ...
        apifyJobs.filter { !it.id.isNullOrBlank() }.forEach { dto ->
            try {
                ...
                val seniorityLevel = parser.extractSeniority(title, dto.seniorityLevel)
                ...
                rawEntries.add(
                        RawJobEntry(
                                ...
                                description = dto.descriptionHtml?.ifBlank { null }
                                                ?: dto.descriptionText,
                                technologies = techs,
                                salaryMin = parser.parseSalary(dto.salaryInfo?.firstOrNull()),
                                salaryMax = parser.parseSalary(dto.salaryInfo?.lastOrNull()),
                                postedDate = parser.parseDate(dto.postedAt),
                                benefits = dto.benefits,
                                employmentType = dto.employmentType,
                                workModel = parser.extractWorkModel(location, title),
        ...
        val jobs =
                groups.values.map { group ->
                    ...
                    JobRecord(
                            ...
                            source = "LinkedIn",
                            country = parser.determineCountry(first.rawLocation),
                            title = first.title,
