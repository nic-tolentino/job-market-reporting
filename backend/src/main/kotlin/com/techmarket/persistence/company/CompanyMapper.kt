    ...
    fun mapCompanyProfile(
        ...
        val roles =
                jobsResult.values.map { r ->
                    ...
                    val locationList =
                            if (r.get(JobFields.LOCATIONS).isNull) emptyList<String>()
                            else r.get(JobFields.LOCATIONS).repeatedValue.map { it.stringValue }
                    val jobIdList =
                            if (r.get(JobFields.JOB_IDS).isNull) emptyList<String>()
                            else r.get(JobFields.JOB_IDS).repeatedValue.map { it.stringValue }
                    val applyUrlList =
        ...
        val allTechs =
                if (detRow?.get(CompanyFields.TECHNOLOGIES)?.isNull == false)
                        detRow.get(CompanyFields.TECHNOLOGIES).repeatedValue.map { it.stringValue }
                else emptyList()
        val hiringLocations =
