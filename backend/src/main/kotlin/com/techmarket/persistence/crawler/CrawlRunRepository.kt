...
class CrawlRunRepository(
    ...
    fun append(record: CrawlRunRecord) {
        val row = mapOf(
            ...
            CrawlRunFields.DETAIL_PAGES_ATTEMPTED to record.detailPagesAttempted,
            CrawlRunFields.DETAIL_PAGES_ENRICHED to record.detailPagesEnriched,
            CrawlRunFields.DESCRIPTION_COVERAGE to record.descriptionCoverage,
    ...
    private fun mapRow(row: FieldValueList): CrawlRunRecord = CrawlRunRecord(
        ...
        detailPagesAttempted = row[CrawlRunFields.DETAIL_PAGES_ATTEMPTED].takeUnless { it.isNull }?.longValue?.toInt(),
        detailPagesEnriched = row[CrawlRunFields.DETAIL_PAGES_ENRICHED].takeUnless { it.isNull }?.longValue?.toInt(),
        descriptionCoverage = row[CrawlRunFields.DESCRIPTION_COVERAGE].takeUnless { it.isNull }?.doubleValue,
